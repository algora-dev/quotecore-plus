                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>{areaComps.map(c => {
                    ...
                    <tr key={c.id} className="border-b border-slate-100">
                      ...
                      <td className="py-3 text-right">{c.priced_quantity != null ? (() => { const ps = (c as { pack_size_snapshot?: number | null }).pack_size_snapshot ?? null; const frac = ps && ps > 0 ? displayQty / ps : displayQty; return <>{c.priced_quantity!.toFixed(0)} <span className="italic text-slate-400">({frac.toFixed(2)})</span></>; })() : (<>{displayQty.toFixed(1)} {getUnitLabel(c.measurement_type as any, quote.measurement_system)}</>)}</td>
