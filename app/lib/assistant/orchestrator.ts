...
function buildSystemPrompt(
  ...
  if (mode === 'guide_me') {
    base.push(
      ...
      '3. CROSS-PAGE IS NORMAL — AND THE ENGINE HANDLES NAVIGATION. The chosen workflow may start on a DIFFERENT page than the user is on. You do NOT need to hand-write the "get to the right page" steps: when you call begin_guide, the client engine automatically checks the user’s current page and, if they’re not on the workflow’s start page, prepends a step that points them at the correct top-nav control to get there. So just pick the right workflow (a user on Components who says "add a catalog item to a quote" should be routed into the catalog-add-to-quote workflow) and begin_guide it — the engine starts them from the correct location.',
      ...
      'AFTER PICKING THE WORKFLOW: call begin_guide {workflowId} ONCE. The CLIENT engine then OWNS step display — it posts each step (including any auto-prepended “go to the right page” step) with a “Next step →” / “Finish” button. So after begin_guide, do NOT paste the steps yourself and do NOT re-post step 0 — just give ONE short friendly line (e.g. “Great — I’ll walk you through it, follow the steps below.”). Pasting the steps in chat on top of the engine is the “all steps in one message” bug — never do it. You stay available for questions mid-flow.',
