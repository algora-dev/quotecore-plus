...
export async function lookupRecovery(oldEmail: string): Promise<LookupResult> {
  ...
  if (!(await checkRateLimit(`recovery-lookup-ip:${ip}`, PER_IP_HOUR_LIMIT, 60 * 60 * 1000, { failClosed: true }))) {
  ...
  if (!(await checkRateLimit(`recovery-lookup-email:${trimmed}`, PER_EMAIL_HOUR_LIMIT, 60 * 60 * 1000, { failClosed: true }))) {
    await logAttempt({ userId: null, oldEmail: trimmed, outcome: 'rate_limited' });
...
export async function verifyRecoveryAnswers(answers: { slot: number; answer: string }[]): Promise<VerifyResult> {
  ...
  const hdrs = await headers();
  const ip = getClientIP(hdrs);
  if (!(await checkRateLimit(`recovery-verify-ip:${ip}`, PER_IP_HOUR_LIMIT, 60 * 60 * 1000, { failClosed: true }))) {
  ...
  if (!(await checkRateLimit(`recovery-verify-user:${payload.sub}`, PER_EMAIL_HOUR_LIMIT, 60 * 60 * 1000, { failClosed: true }))) {
    await logAttempt({ userId: payload.sub, oldEmail: null, outcome: 'rate_limited' });
