...
import type { CalibrationCompanyRow } from './actions';
import { setCalibrationEnabled } from './actions';

export function CalibrationFlagPanel({ initialRows }: { initialRows: CalibrationCompanyRow[] }) {
  ...
  function toggle(row: CalibrationCompanyRow) {
    setBusyId(row.company_id);
    setError(null);
    startTransition(async () => {
      const result = await setCalibrationEnabled(row.company_id, !row.enabled);
      if (result.ok) {
        setRows((prev) =>
          prev.map((r) =>
