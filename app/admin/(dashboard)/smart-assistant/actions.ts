...
export type SmartAssistantCompanyRow = {
  company_id: string;
  name: string;
  slug: string | null;
...
export async function listSmartAssistantCompanies(
  filter?: string,
): Promise<{ ok: true; rows: SmartAssistantCompanyRow[] } | { ok: false; error: string }> {
  await requireAdmin();
  ...
  let rows: SmartAssistantCompanyRow[] = (companyRows ?? []).map((c) => {
  ...
  // Enabled companies first so the rollout list is visible without scrolling.
  rows.sort((a, b) => {
...
export async function setSmartAssistantEnabled(
  companyId: string,
  enabled: boolean,
  ...
  await writeAudit(
    admin,
    adminProfile,
    enabled ? 'smart_assistant_enabled' : 'smart_assistant_disabled',
    companyId,
    null,
    null,
    null,
