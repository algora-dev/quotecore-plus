---
title: Digital takeoff
description: Upload a plan or image and measure areas, lengths, and items directly on screen.
order: 3
status: published
updated: 2026-07-30
---

# Digital takeoff

Upload any scaled plan or image and measure directly on screen. Set a calibration reference on a known measurement and you can measure areas, lineal lengths, and point-based items without manual scaling or guesswork.

All you need is a reliable scale or defined measurement on that plan that you know is correct. In digital takeoff, the first step is calibrating the measurement system using that scale.

So long as you are confident in the accuracy of that measurement, you can essentially measure anything that is an area or a lineal item. Tip: use the longest available measurement on the plan (e.g. a full wall run or site boundary) for the most accurate calibration.

## What you can measure

- Multiple areas or sections, each with its own settings.
- Individual components by area, lineal length, lineal x height (area), or as **points** for fixed or quantity-based components.

### Area tools

When you click the **Area** button, two sub-tools are available:

- **Polygon** — click points to outline any shape (at least 3 points). Click back on the first point to close the area. Best for irregular shapes.
- **Rectangle** — click and hold, drag, then release to create a square or rectangular area. Fastest for uniform shapes.

Both tools produce the same result: a measured area stored in your calibration units (metric, imperial, or roofing squares) and attached to a roof area or component.

All measurements are saved and auto-calculated to pre-populate the next phase in the quote builder.

### Per-entry pitch

Each area measurement can have its own pitch angle. When you measure multiple areas on the same plan, each one can have a different pitch — the app tracks pitch per measurement entry, not per area. Components inherit the pitch of the area they belong to.

The pitch input supports degrees, ratio, and gradient modes (see [Waste, material costs, and pitch](/docs/concepts/waste-and-pitch)).

### Canvas tools

- **Undo / Redo** — step backward and forward through your drawing actions.
- **Page-scoped drawing** — measurements and drawings are tied to the page you're on. Switching pages won't leak drawings across pages.
- **Multi-page support** — upload multiple plans and measure on each independently. Calibrations are saved per page.
- **Component assignment** — assign measurements to specific components as you draw. Use "+ New Area" to create roof areas without a component.

## AI Assist (BETA)

AI Assist uses vision AI to automatically trace your roof plan - areas, ridges, hips, valleys, barges, and spouting - in one continuous flow. It is available for **roofing trade** companies on **Growth plan and above**.

### Quick start

1. Upload your plan and **calibrate the scale**.
2. Select a **quality level** (Low, Medium, or High).
3. Click **AI Assist (BETA)**.
4. The AI runs a 3-scan pipeline (perimeter trace, internal line trace, line classification) automatically.
5. Review the results modal - name each area and set pitch angles.
6. Click **Apply** to send the traced areas and lines to your canvas.

### Quality levels

- **Low** - fastest, good for simple roofs.
- **Medium** (default) - balanced for most roofs.
- **High** - most detailed, best for complex multi-gable roofs.

After applying, you can continue measuring manually or proceed to the quote builder. AI Assist is a starting point - review and adjust on the canvas as needed.

For the full guide, see [AI Assist takeoff](/docs/building-a-quote/ai-takeoff).

## What happens next

When you continue from digital takeoff into the [quote builder](/docs/building-a-quote/quote-builder), you will see each area you created, each component you measured, and its associated items. You can edit and adjust if needed, or continue quickly to the next step.

## What gets saved

- Your initial image or plan.
- A digital takeoff version of the image showing your lines, measurements, areas, and items overlaid on the plan.
- A clean image without your uploaded image and only your digital takeoff lines / items, so you can clearly see what you measured.

These are useful for printing out and doing site measures, or to clearly define what you actually measured at a later point if you need to reference it.

## Related

- [Quote builder](/docs/building-a-quote/quote-builder)
- [Manual Quote](/docs/building-a-quote/manual-quote) - if you already have your measurements.
- [Blank Quote](/docs/building-a-quote/blank-quote) - for fully custom line-by-line quotes without takeoff.
