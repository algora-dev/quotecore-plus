...
When you add a component that uses fixed-quantity pricing (per pack, per item, etc.), the line shows a single **Quantity** value with the unit of measurement — instead of expanding the full measurement breakdown. This keeps the order clean when the per-unit breakdown isn't relevant.

For per-unit components (priced per m², per metre, etc.), the full measurement breakdown (lengths, areas, entries) is still shown as usual.

Quantity values edited in the order editor are stored on the order line only — they never write back to the quote or component library.
