...
Pitch fields support three input modes — toggle between them with the buttons next to the field:

- **Degrees (°)** — enter the angle directly (e.g. 30°).
- **Ratio (1:X)** — enter the run for every 1 unit of rise (e.g. 1:1.73 for 30°).
- **Gradient (%)** — enter the slope as a percentage (e.g. 57.7% for 30°).

The app converts between modes automatically, so you can switch at any time without losing the value.
