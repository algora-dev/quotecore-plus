-- 20260526160000_fix_save_takeoff_atomic_overload.sql
--
-- HOTFIX: Migration 20260526150000 accidentally used `p_payload json`
-- instead of `p_payload jsonb`. PostgreSQL treated this as a NEW overload
-- rather than replacing the existing `(uuid, jsonb)` version. With two
-- overloads differing only by json vs jsonb, every RPC call became
-- ambiguous and failed with "Server Components render" error on save.
--
-- This migration:
--   1. Drops the broken `json` overload.
--   2. Correctly replaces the `jsonb` version, adding the version guard
--      and advisory lock from 20260526150000 but with all the original
--      body intact (pre-flight checks, v_new_component_id, entry insertion
--      for new components, etc.).

-- 1. Drop the broken overload
DROP FUNCTION IF EXISTS public.save_takeoff_atomic(uuid, json);

-- 2. Replace the correct jsonb overload with version guard + advisory lock
...
DECLARE
  ...
  v_submitted_version integer := (p_payload->>'session_version')::integer; -- nullable
  v_component         jsonb;
...
BEGIN
  ...
  -- ── 3. Optimistic version check (when client submits a version) ───────────
  ...
  -- ── 4. H-01: component compatibility pre-flight (fail fast before writes) ─
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  'Round-8 + P1-1a: pre-flight H-01 compatibility guard + page-scoped delete '
  'pg_advisory_xact_lock per quote + optimistic version check + version increment.';
