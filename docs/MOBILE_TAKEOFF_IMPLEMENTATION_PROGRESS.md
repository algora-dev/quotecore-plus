...
## M8 — Owner-prescribed touch UX overhaul (2026-09-21)

Owner live iPhone test (16:53) judged the M7 shell too busy: canvas not full-bleed, controls spread across top/bottom strips, AI reachable before calibration, no name/pitch prompt after accepting an outline. This phase implements the owner prescription exactly; it supersedes the s3 top/bottom-strip layout for the touch presentation only. **Refinement (16:59):** NO Calibrate/Outline/Menu step-switching tab row — the rail shows ONLY the current step's controls and the flow itself drives which control set is shown (calibration phase → calibration controls; advances to outline controls — manual draw + AI scan offer + name/pitch on accept — once calibration completes). A compact current-step label sits at the rail top next to a small Menu (⋯) button.

Status: complete per the M8 task scope. Desktop presentation and flag-off paths bit-for-bit untouched (L09/L10).

Existing modules reused: M3 gesture stack, M4 calibration workspace (controller + EndpointControllerRail + CalibrationSheet), M5 touchOutlines/TakeoffWorkstation adapter (handleSaveArea with pointsOverride — the desktop name/pitch data path), M6 touchAiOutline.

New / changed modules:
- `TouchWorkspaceShell.tsx` — FULL-BLEED canvas (edge-to-edge, full height, no top/bottom strips) + single WIDER (~160px, w-40) right rail. M8 16:59: flow-driven — compact current-step label + Menu (⋯) button at the rail top; the Menu panel holds the view-mode switch, Back (O16 dirty-draft guard), plan label and compact notices (L08). Rail content scrolls internally (never overflows); safe-area side inset kept. Portrait hint floats over the canvas. Mount-stability contract and desktop `w-[125%] -ml-[12.5%]` inert skeleton preserved exactly.
- `TouchOutlineEditor.tsx` — outline panel (status, area chips, +New, AI scan, compact ↶/↷ undo/redo icons, Close/Save/Cancel) moved INTO the rail (bottom strip no longer exists); M8 post-accept flow: closing a NEW outline immediately prompts roof-area NAME + PITCH, and the AI-offer Continue now opens the same prompt (AI name/pitch prefill only) — both confirm through adapter.createOutline → handleSaveArea, so mobile-created areas are identical to desktop-created ones; `outlineScanCalibrationGate` wired into both the scan chip and startScan.
- `touchAiOutline.ts` (+tests) — pure `outlineScanCalibrationGate(scale)`: HARD GATE — the AI outline scan is unavailable until the current page has a completed calibration (null/non-finite/non-positive scale blocks with a surfaced reason).
- `TakeoffPage.tsx` — flow-driven phase (16:59): an uncalibrated page opens in the calibration phase; the calibration sheet's Done (or a completed calibration on re-entry) advances to the outline controls — no step-switching UI. MANUAL-ONLY calibration in touch view (`aiEnabled: false` — AI calibration entry hidden in touch presentation; desktop unchanged).
- `e2e/specs/takeoff-touch-precision.spec.ts` — assertions updated to the rail/tab structure; new M8 assertions: scan chip disabled before calibration, post-accept name/pitch prompt on close.

Schema / compatibility impact: none (no migrations, no server changes).

Commands run and results: `npm run test:precision` → 146 pass / 0 fail (143 + 3 gate tests); `npm run test:calibration` → 223 pass / 0 fail; eslint on all touched files → 0 problems; `next build` → success; `npx playwright test -c playwright.touch.config.ts` → 4 passed / 0 failed (2 × touch-chromium, 2 × touch-webkit; full journeys incl. tabbed-rail assertions, pre-calibration scan-gate disabled state, post-accept name/pitch prompt, guards, mocked scan, 568×320 floor).

Physical / live-service tests not run: on-device iPhone re-test (owner); AI calibration hiding verified by code path only (aiEnabled:false hides the sheet entry).

Decisions or deviations, with reason:
1. Back lives in the Menu panel (prescription: "Back/menu"); the O16 guard still fires from there. With the 16:59 refinement there is NO way back to calibration once the flow advances (a calibrated page re-enters straight to outline) — recalibration of an already-calibrated page from touch requires the desktop view (consistent with the existing C13 dependents gate); flag if the owner wants an "Edit scale" action in the outline rail.
2. The calibration distance sheet still floats over the canvas (form content, not chrome); owner M7.1 sizing fixes preserved.
3. `confirmCreate` now hands the accepted draft's points directly to the create flow (draft closed + blocking-review clean) instead of re-running outlineSaveIntent — required because an unchanged AI import returns a `clean` boundary verdict, and keeps one path for manual + AI accepts.
4. Rail width fixed at 160 CSS px (within the prescribed 150-170px; ~28% of 568px landscape, comfortably fitting at 568x320 with a 396px canvas).
5. `next.config.ts` gains `devIndicators: false` — the Next dev-tools badge sits in the bottom-right corner and intercepted the full-bleed touch layout's floating sheet buttons (calibration Done) in the local e2e harness. DEV-ONLY setting; production builds are unaffected. The touch spec also hides `nextjs-portal` overlays defensively (test chrome, not app UI).
