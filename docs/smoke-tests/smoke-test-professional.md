...
> Major failure: Not only did opening a new digital take off session remove the previously logged data, it didn't add up the new measurement correctly either, something in the logic is not working here please inspect. I think what actually NEEDs to happen when a user has created/saved a digital takeoff quote, and they want to do another digital take off, the user should be asked if they want to create a new "area" (technically this would be a second page upload flow) or if they want to add to the current area/original takeoff measurements - The key with the latter is the new measurements need to be added on top of the original take off, not replace them.
...
> Works but we need to stop allowing full URL text links and enforce hyperlnk text "Click Here" options if a user wants to add a second link other than the button link. This needs to happen at template creation, OR when the user manually types a message/email and adds a full URL. Warn them that its unprofessional and will likely flag the email and send to junk/spam - Please use the hyperlink system (Create a new drop in hyperlink system that allows the user to add a full URL, and then define the actual hyperlink text that will go in the message for example; "full URL here", actual shown text = Click Here, or something the user chooses, and that option is a click and drop in, the URL can obviously only be something generated by our system, like a quote, or order link, the customer selects the URL, then defines the text hyperlink that shows in the message/email)
...
> no post send follow up doesn't appear for pro I don't think? It didn't pop up either way, but I thought follow up messages are for higher tier? If not, you need to fix it. Make sure Pro tier actually has follow up system access.
...
> Couldn't test the follow up items because of above #6 issue. Everything else working.
...
> Works for roofing, but for other trades Drawings/images page button (on components page) needs to change to "Upload/Draw Images" or something generic, not "Flashings", or we keep in Flashings for roofing, and "Upload/Draw Images" for all other trades. 
...
> Working, but confirm order button on the actual order is not doing anything (should send alert to user that the order has been received/confirmed) This system will need to tie into our project manager mode later to auto update the status etc, need to make sure the buttons connected to order URLs all actually work/do something.
...
> Working, but only option to cancel is by clicking on "manage subscription" on billing page, that goes straight to stripe. I have a screenshot of that if you want? We need to allow the user to view current subscription, and cancel from there too. So, in this example, the Professional tier shows "Current" in green text, it allows the user to click the "View" button, but shows "your current plan" as the other button, which cannot be clicked. Lets change that to "Cancel" and make it clickable, it should take the user directly to the cancel subscription stripe page, just like it would if they go the long way,  "manage subscription". Also, now getting "No such customer: 'cus_UXWHxulYOoHgHJ'; a similar object exists in test mode, but a live mode key was used to make this request." on the Professional Tier account, The starter tier account works and goes to the stripe page as designed. I assume this is something to do with what you mentioned about the pro tier having a possible issue.
