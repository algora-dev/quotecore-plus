-- Migration: 20260705230000_rpc_v5_area_scoped_measurement_delete.sql
-- Purpose: Fix "duplicate areas in first parent area" (quote c7a973e1 repro).
--
-- Root cause chain (2026-07-05 diagnosis):
--   1. Client: rows drawn before initializeTakeoffPage resolves have
--      fromPageId=null and can be re-homed to the wrong page by later saves
--      (fixed client-side in TakeoffWorkstation: retro-stamp on id resolve).
--   2. RPC v4 Section 6: the ROOF-AREA measurement delete was page-scoped
--      but NOT area-scoped. A per-area flush save (e.g. Garage on Page 1)
--      deleted ANOTHER area's (Main Roof's) area-measurement rows on that
--      page, while Section 8's entry delete IS area-scoped -- leaving Main
--      Roof's quote_roof_area_entries on Page 1 orphaned (no backing
--      measurements). Those orphans are the duplicate #2/#4 rows in the
--      quote builder.
--
-- Change (v5): Section 6 roof-area measurement delete now mirrors the
-- component-measurement delete: when v_target_roof_area is provided the
-- delete is scoped to that area, so one area's save can never destroy
-- another area's measurements on the same page.
--
-- Everything else is byte-identical to v4 (migration 20260705220000).
...
BEGIN
  ...
  -- Roof area measurements (v5: area-scoped when target provided, mirroring
  -- the component delete above. v4's page-only scoping let a per-area flush
  -- save delete ANOTHER area's measurement rows on the same page, orphaning
  -- that area's quote_roof_area_entries -> duplicate areas in quote builder.)
  IF v_target_roof_area IS NOT NULL AND v_current_page_id IS NOT NULL THEN
    DELETE FROM public.quote_takeoff_measurements
     WHERE quote_id = p_quote_id
       AND quote_roof_area_id = v_target_roof_area
