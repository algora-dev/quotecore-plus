-- Migration: 20260705160000_per_area_quote_components.sql
-- Purpose (Area-ownership fix, 2026-07-05): save_takeoff_atomic assigned ALL
--   quote_components in a save to ONE area (coalesce(v_target_roof_area,
--   v_first_roof_area)) and looked up existing rows by (quote, component)
--   only — so the same component could never exist in two areas, and every
--   component in a multi-area takeoff landed under a single area (the
--   "everything moved to Garage" bug, Shaun 2026-07-05).
-- Fix:
--   1. Components payload elements may carry quote_roof_area_id.
--   2. Existing-row lookup becomes (quote, component, area) via
--      IS NOT DISTINCT FROM, so per-area component rows are supported.
--   3. Insert stamps the component's own area id.
--   4. Validate payload area ids belong to this quote (defence in depth).
-- Rollback: re-apply 20260705120000_page_scoped_area_entries.sql.
...
BEGIN
  ...
  -- Guard: COALESCE null/missing measurement_value to 0 (NOT NULL constraint).
  IF jsonb_array_length(v_measurements) > 0 THEN
    ...
    -- Area-ownership fix (2026-07-05): payload roof-area ids must belong to
    -- this quote (defence in depth — client bugs or tampering).
    IF EXISTS (
      SELECT 1
        FROM jsonb_array_elements(v_measurements) AS m
       WHERE nullif(m->>'quote_roof_area_id', '') IS NOT NULL
         AND NOT EXISTS (
           SELECT 1 FROM public.quote_roof_areas ra
            WHERE ra.id = nullif(m->>'quote_roof_area_id', '')::uuid
              AND ra.quote_id = p_quote_id
         )
    ) THEN
      RAISE EXCEPTION 'One or more quote_roof_area_id values do not belong to quote %', p_quote_id;
    END IF;
  ...
  -- RC-6 fix (2026-07-05): the first-touch delete is now PAGE-SCOPED so a save
  -- from one plan page no longer destroys entries created from other pages.
  -- NULL-page rows (pre-migration legacy) are treated as current-page rows and
  -- re-stamped on the next save.
  IF jsonb_array_length(v_roof_areas) > 0 THEN
    ...
    LOOP
      ...
      IF v_area_id IS NOT NULL THEN
        -- First touch in this save: clear previous takeoff entries FOR THIS
        -- PAGE ONLY (RC-6). Other pages' entries survive.
  ...
  FOR v_component IN SELECT * FROM jsonb_array_elements(v_components) LOOP
    ...
    IF v_comp_area IS NOT NULL AND NOT EXISTS (
      ...
      RAISE EXCEPTION 'Component roof area % does not belong to quote %', v_comp_area, p_quote_id;
    END IF;
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  'Fix 2026-07-05 (area ownership): per-area quote_components — lookup/insert '
  'by (quote, component, area) with payload-supplied quote_roof_area_id, plus '
  'payload area-id validation. Prior: RC-6 page-scoped area entries.';
