-- Migration: 20260705120000_page_scoped_area_entries.sql
-- Purpose (RC-6, 2026-07-05 Fable 5 audit): save_takeoff_atomic deleted ALL
--   quote_roof_area_entries with source='takeoff' for an area on first touch
--   in any save (not page-scoped). Partial client payloads (existing-area mode
--   sends only newly drawn areas) therefore destroyed other pages' entries —
--   the "Main House overwritten to 0.00" bug.
-- Fix:
--   1. Add nullable page_id to quote_roof_area_entries (additive).
--   2. Scope the first-touch delete to the current page (NULL-page legacy rows
--      are treated as current-page rows, mirroring the section-6 measurement
--      delete idiom, and are re-stamped on next save — self-healing).
--   3. Stamp page_id on insert.
-- Rollback: re-apply 20260704200000_guard_measurement_value.sql (column can stay).
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  'Fix 2026-07-05 (RC-6): page-scoped quote_roof_area_entries delete + page_id '
  'stamping so partial saves from one plan page no longer destroy other pages'' '
  'takeoff entries. Prior: 2026-07-04 v3 NaN/null COALESCE guards.';
