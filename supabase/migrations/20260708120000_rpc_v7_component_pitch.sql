-- Migration: 20260708120000_rpc_v7_component_pitch.sql
-- Purpose: Components carry the pitch they were calculated at.
--
-- Context (2026-07-08): components were pitched at the FIRST roof-area
-- measurement's pitch on the page (firstRoofAreaPitch), regardless of which
-- area they belong to, and quote_components.calc_pitch_degrees was never
-- written (always NULL) so the calc audit reported pitchDegrees=0 while the
-- stored value_after_waste silently included a pitch factor.
--
-- Changes from v6 (20260706130000):
--   NEW: quote_component_entries.pitch_degrees column (additive, nullable).
--   Section 9 (existing component): entry INSERT includes per-entry
--     pitch_degrees from payload; component UPDATE also sets
--     calc_pitch_degrees from payload when provided.
--   Section 9 (new component): INSERT includes calc_pitch_degrees;
--     entry INSERT includes per-entry pitch_degrees.
--
