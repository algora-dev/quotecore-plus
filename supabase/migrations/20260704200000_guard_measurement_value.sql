-- Migration: 20260704200000_guard_measurement_value.sql
-- Purpose: Guard against null measurement_value in save_takeoff_atomic INSERT.
--   If the JSON payload has null/missing measurement_value (e.g. from NaN serialization),
--   COALESCE to 0 instead of violating the NOT NULL constraint.
-- Rollback: re-apply 20260704160000_fix_takeoff_area_entries.sql
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  'Fix 2026-07-04 v3: COALESCE guards on measurement_value + component entries '
  'to prevent NOT NULL violations from NaN/null serialization. '
  'Also guards raw_value and value_after_waste in component entries.';
