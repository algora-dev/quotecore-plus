-- Migration: 20260704160000_fix_takeoff_area_entries.sql
-- Purpose: Fix two bugs in save_takeoff_atomic (introduced in 20260704130000)
--   BUG 1 (sqm=0): the loop used `v_roof_areas[v_idx]` with a 1-based FOR loop,
--     but Postgres jsonb subscripting is 0-BASED. For a 1-element payload,
--     v_roof_areas[1] returned NULL, so computed_sqm coalesced to 0 and the
--     entry was inserted with sqm=0 via the target-area path.
--     Fix: iterate with jsonb_array_elements(...) WITH ORDINALITY.
--   BUG 2 (entry accumulation): repeated saves (auto-save on area switch +
--     full save) appended new quote_roof_area_entries rows without clearing
--     the previous takeoff-created ones, inflating/duplicating entries.
--     Fix: add `source` column to quote_roof_area_entries; takeoff writes
--     source='takeoff' and deletes prior takeoff entries per area before
--     inserting. Manual quote-builder entries (source='manual') are preserved.
--   Also: each roof_areas payload element now carries quote_roof_area_id so
--     the RPC resolves the area row directly (validated against the quote)
--     instead of relying on label matching / first-ordinal heuristics.
-- Rollback: re-apply 20260704130000_takeoff_area_submeasurements.sql
--   (and optionally drop the source column).

-- 1. Source column (additive, nullable-default — safe on live DB)
ALTER TABLE public.quote_roof_area_entries
  ADD COLUMN IF NOT EXISTS source text NOT NULL DEFAULT 'manual';

-- Backfill: entries created by the takeoff RPC always have width_m=0 AND
-- length_m=0 (polygon areas have no W×L). Manual quote-builder entries
-- always have positive W×L (the UI requires w>0 && l>0). Safe to tag.
UPDATE public.quote_roof_area_entries
   SET source = 'takeoff'
 WHERE width_m = 0 AND length_m = 0;
...
BEGIN
  ...
  -- Areas are created upfront by createNewTakeoffArea; the RPC never inserts
  -- quote_roof_areas rows. For each drawn polygon in the payload:
  --   a) Resolve the area row: explicit quote_roof_area_id (validated) →
  --      target_roof_area (first ordinal) → label match.
  --   b) On first touch of an area in this save, delete its previous
  --      takeoff-created entries (source='takeoff'); manual entries survive.
  --   c) Insert a source='takeoff' entry with the polygon's metric area.
  --   d) Recompute the parent area's totals from ALL remaining entries.
  IF jsonb_array_length(v_roof_areas) > 0 THEN
    ...
    LOOP
      ...
      IF v_area_id IS NOT NULL THEN
        -- First touch in this save: clear previous takeoff entries.
        ...
        INSERT INTO public.quote_roof_area_entries (
          quote_roof_area_id, width_m, length_m, sqm, sort_order, source
        ) VALUES (
          v_area_id, 0, 0, v_area_sqm, v_area_ord::int, 'takeoff'
        );
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  'Fix 2026-07-04: WITH ORDINALITY iteration (jsonb subscripting is 0-based), '
  'per-element quote_roof_area_id resolution, source=takeoff entry replacement. '
  'Areas created upfront by createNewTakeoffArea; RPC never inserts area rows.';
