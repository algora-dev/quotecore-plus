-- Migration: 20260705190000_fix_rpc_columns_and_soft_validation.sql
-- Purpose (2026-07-05): The 20260705180000 migration rewrote save_takeoff_atomic
--   from scratch with MANY incorrect column names (default_* prefixes on
--   quote_components, calc_pitch_degrees on quote_roof_area_entries,
--   measurement_type/value_after_pitch/material_cost/quote_roof_area_id on
--   quote_component_entries). This caused cascading errors:
--   1. "column v_caller_uid does not exist" (param was p_caller_uid)
--   2. "column calc_pitch_degrees of relation quote_roof_area_entries does not exist"
--   3. (would have hit more if we got further)
--   Fix: revert to the EXACT working function body from 20260705160000, then
--   apply ONLY the soft validation change:
--   - Replace RAISE EXCEPTION for invalid quote_roof_area_id in measurements
--     with a soft cleanup (null out invalid values, let coalesce handle them)
--   - Soften component area validation to fall back to first roof area
--   Keep the 2-arg signature (no p_caller_uid param) to match the client.
-- Rollback: re-apply 20260705160000_per_area_quote_components.sql

-- Drop BOTH existing signatures to avoid ambiguity.
DROP FUNCTION IF EXISTS public.save_takeoff_atomic(uuid, jsonb);
DROP FUNCTION IF EXISTS public.save_takeoff_atomic(uuid, jsonb, uuid);
...
BEGIN
  ...
  -- Instead of RAISE EXCEPTION (which blocked the entire save), null out any
  -- quote_roof_area_id that doesn't belong to this quote. The coalesce in the
  -- INSERT then falls back to v_target_roof_area.
        ...
        BEGIN
          IF v_m_area_id::uuid != ALL(COALESCE(v_valid_area_ids, ARRAY[]::uuid[])) THEN
            v_measurements := jsonb_set(
              v_measurements,
              ARRAY[v_m_idx::text],
              jsonb_set(v_m, '{quote_roof_area_id}', 'null'::jsonb, false),
              false
            );
          END IF;
  ...
  FOR v_component IN SELECT * FROM jsonb_array_elements(v_components) LOOP
    ...
    -- Soft validation: fall back to first roof area instead of raising.
...
COMMENT ON FUNCTION public.save_takeoff_atomic(uuid, jsonb) IS
  ...
  'measurements are nulled out instead of raising. Component area validation '
  'falls back to first roof area. Uses correct column names from '
  '20260705160000. Prior: broken rewrite in 20260705180000.';
